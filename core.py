...
def build_technical_architecture_pdf(report_data):
    ...
    story.append(Paragraph("Generated by Technical Document Reader", body_style))
    story.append(Spacer(1, 0.15 * inch))
            ...
            if value:
                for item in value:
                    if isinstance(item, dict):
                        ...
                        story.append(Paragraph(f"• {line or '-'}", body_style))
                    else:
    ...
    design_flow = report_data.get("design_flow") or []
    story.append(Paragraph("Design Flow", section_style))
    if design_flow:
        for idx, item in enumerate(design_flow, start=1):
    ...
    if glossary:
        rows = [[Paragraph("<b>Term</b>", body_style), Paragraph("<b>Meaning</b>", body_style)]]
        for item in glossary:
            rows.append([
                Paragraph(str(item.get("term", "") or "-"), body_style),
                Paragraph(str(item.get("meaning", "") or "-"), body_style),
            ])
        glossary_table = Table(rows, colWidths=[2.0 * inch, 4.3 * inch])
